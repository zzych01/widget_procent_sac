 ...
 #progress-spinner {
  ...
  class Widget extends HTMLElement {
    ...
    onCustomWidgetAfterUpdate(changedProperties) {
      // Handle data binding from percentage_1
      if ("percentage_1" in changedProperties) {
        const dataBinding = changedProperties.percentage_1;
        if (dataBinding.state === 'success' && dataBinding.data) {
          this._updateDataFromBinding(dataBinding);
        }
      }
    ...
    _updateDataFromBinding(dataBinding) {
      if (dataBinding && Array.isArray(dataBinding.data)) {
        const row = dataBinding.data[0];
        if (row && row.measures_0) {
          // Extract the measure value and update percentage
          this._props.percentage = Math.round(row.measures_0.raw);
        }
      }
    }
  }
